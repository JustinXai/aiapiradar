...
export const pages: PageData[] = [
  ...
  // 3. shipin-shengcheng-api (NEW FULL)
  {
    ...
    whenToUseLinkAI:
      "在 LinkAI 查看是否支持目标视频生成模型，注册后在 LinkAI 获取 API Key，用小额视频生成任务测试实际扣费和可用性。",
    aiSummary:
  ...
  // 8. openclaw-api-key
  {
    ...
    title: "OpenClaw API Key 配置与排错指南：401、403、credits 与安全风险",
    metaDescription:
      "系统介绍 OpenClaw API Key 的配置与排错，解释 OpenClaw、OpenRouter、API Key、credits、模型权限的关系，说明 401/403/no credits/model not found/rate limit/timeout 等常见错误，以及 Token 成本、安全风险和小额测试策略。",
    h1: "OpenClaw API Key 配置与排错指南",
    pageType: "guide",
    status: "full",
    quickAnswer:
      "OpenClaw 通常通过兼容 OpenAI 接口的后端服务完成认证，实际使用时要同时确认 API Key 是否有效、credits 或预算是否足够、模型是否可见、Base URL 是否正确。出现 401、403、no credits、model not found 或 timeout 时，建议先核对 key、/v1/models、usage 和服务商后台记录，再决定是否更换配置或做小额测试。",
    audience: [
      "第一次配置 OpenClaw API Key 的开发者",
      "遇到 401、403、no credits、model not found 的用户",
      "关心 OpenClaw 凭证安全、Token 成本和扣费透明的人",
    ],
    conceptExplanation:
      "OpenClaw、OpenRouter、API Key、credits 和模型权限是一个连在一起的调用链：\n\n- **OpenClaw**：AI Agent 客户端，本身负责多轮对话、工具调用、文件读取和任务编排。\n- **OpenRouter 或其他兼容后端**：提供模型聚合、路由和账户结算能力。\n- **API Key**：身份认证凭证，服务商据此识别账户、应用权限和可调用模型范围。\n- **Credits / budget**：服务商用于计费或限额控制的余额机制。即使 key 正确，如果 credits 不足、预算上限触发或账户被限制，也可能报 403 或 no credits。\n- **模型权限**：某些模型需要单独开通、白名单或更高等级账户。即使 API Key 有效，也不代表目标模型一定可见。\n\n在 OpenClaw 场景里，问题往往不止出在“key 对不对”。Agent 多轮循环会放大请求次数和上下文长度，所以需要把认证、余额、模型权限、Base URL、日志和 usage 一起看。用 `/v1/models` 或服务商控制台确认模型是否可见，是比单纯猜测更稳妥的做法。",
    setupOrCheckSteps: [
      "确认你使用的是哪一类后端：OpenRouter、其他兼容 OpenAI 接口的服务商，还是自建中转；记录对应的 API Key 和 Base URL",
      "先检查 API Key 是否仍有效，是否已过期、被撤销、被替换，或复制时多了空格和换行",
      "访问 `/v1/models` 或打开服务商控制台，确认目标模型是否真的对当前账户可见",
      "检查 credits、预算上限、套餐额度和账单状态，确认不是 no credits、budget hit 或权限限制",
      "在 OpenClaw 中核对 Base URL、模型名、请求头格式和环境变量名是否与服务商要求一致",
      "查看 request_id、usage、后台调用记录和错误日志，确认失败发生在哪个环节",
      "先做一次最小化请求或小额测试，再扩大到多轮 Agent 任务，避免一开始就把长上下文和工具调用叠加上去",
    ],
    commonErrors: [
    ...
    faq: [
      {
        question: "OpenClaw API Key 一定等于 OpenRouter API Key 吗？",
        answer: "不一定。很多教程会用 OpenRouter 作为 OpenClaw 后端，所以常见情况是填 OpenRouter API Key；但如果你接的是其他兼容 OpenAI 接口的服务商或中转，OpenClaw 使用的就是那个服务商提供的 key。关键不是名字，而是后端、Base URL、模型权限和计费规则是否匹配。",
      },
      {
        question: "为什么 API Key 明明没填错，还是返回 403？",
        answer: "403 不一定代表 key 错了。更常见的是 credits 不足、预算上限触发、账户权限不足、模型未授权，或者服务商对该模型设置了访问限制。先看后台余额和模型权限，再看 request_id 对应的失败记录。",
      },
      {
        question: "出现 no credits 时是不是一定没有扣费？",
        answer: "不一定。若请求在网关前就被拒绝，通常不会产生完整模型费用；但如果请求已进入部分处理流程，是否扣费要以服务商规则和后台记录为准。更稳妥的做法是保存 request_id、usage 和失败时间点，再结合后台账单综合判断。",
      },
      {
        question: "model not found 一定是模型名拼错了吗？",
        answer: "不一定。模型名写错只是其中一种情况。另一些常见原因包括：模型已下架、模型只对特定账户开放、当前 API Key 无权访问，或者你访问的 Base URL 对应的是另一套模型目录。先用 `/v1/models` 或服务商控制台确认模型是否可见。",
      },
      {
        question: "为什么 OpenClaw 比普通聊天更容易把 credits 用得很快？",
        answer: "因为 OpenClaw 是 Agent，不只是单轮提问。它可能连续读取文件、调用工具、重试请求、保留长上下文并进行多轮循环。这些都会累计 Token 和 credits 消耗，所以成本通常高于普通单次聊天请求。",
      },
      {
        question: "API Key 应该放在哪里最安全？",
        answer: "优先放在环境变量、系统密钥管理或权限受控的服务端配置中，不要写死在代码仓库、公开笔记、截图或聊天窗口里。如果必须写入配置文件，至少要限制文件权限，并避免把配置文件同步到公共仓库。",
      },
    ],
    relatedLinks: [
      { title: "OpenClaw 微信配置教程", url: "/openclaw-wechat/", reason: "查看 OpenClaw 在消息场景中的接入方式与额外成本" },
      { title: "OpenClaw 是什么？", url: "/openclaw-shi-shenme/", reason: "先理解 OpenClaw 的定位、调用链和 Agent 特性" },
      { title: "OpenClaw OpenRouter 使用方式", url: "/openclaw-openrouter/", reason: "理解 OpenRouter credits、模型路由和 OpenClaw 的关系" },
      { title: "OpenClaw 安全指南", url: "/openclaw-security/", reason: "补充凭证保护、权限最小化和聊天入口安全" },
      { title: "OpenAI API Base URL 是什么？", url: "/openai-api-base-url/", reason: "核对 Base URL 配置逻辑和兼容接口概念" },
      { title: "Coding Agent Token 成本分析", url: "/coding-agent-token-cost/", reason: "理解 Agent 多轮循环、工具调用和上下文对成本的放大效应" },
      { title: "API 中转站安全吗？", url: "/api-zhongzhuan-safe/", reason: "评估中转服务的透明度、稳定性和安全风险" },
    ],
    primaryCTA: { label: "去 AI API Doctor 检测 API 配置", url: "https://aiapidoctor.com/" },
    secondaryCTA: { label: "Create a LinkAI account", url: "https://api1.link-ai.cc/register" },
    pricingCTA: { label: "查看 LinkAI 模型价格", url: "https://api1.link-ai.cc/pricing" },
  ...
  // 19. mcp-shi-shenme (existing)
  {
    ...
    title: "MCP 是什么意思？Model Context Protocol 原理与用途",
    metaDescription: "MCP 是 Model Context Protocol，用来让 AI 模型连接工具、文件、数据库和服务。本文解释 MCP 的作用、与 API 和插件的区别，以及常见使用场景。",
    h1: "MCP 是什么意思？",
    pageType: "guide",
    status: "scaffold",
    quickAnswer: "MCP 是 Model Context Protocol 的缩写，是一种让 AI 应用连接外部工具、数据和服务的开放协议。配置 MCP 时需要注意 API Key 权限、环境变量安全和服务器来源可靠性。",
  ...
  // 26. claude-code-api-key (existing)
  {
    ...
    whenToUseLinkAI: "在 LinkAI 获取 API Key，注册后进行小额测试。",
  ...
  // 27. claude-code-base-url (existing)
  {
    ...
    whenToUseLinkAI: "在 LinkAI 获取中转 Base URL，注册后进行小额测试。",
  ...
  // 29. claude-code-openrouter (new full)
  {
    ...
    title: "Claude Code OpenRouter 配置：Base URL、API Key 和模型名",
    metaDescription:
      "Claude Code 接入 OpenRouter 的配置方法，包含 Base URL、API Key、模型名、环境变量，以及 401、404、model not found 等常见问题排查。",
    ...
    faq: [
      ...
      {
        question: "OpenRouter 怎么配置 Claude Code？",
        answer: "配置分四步：1. 把 `ANTHROPIC_BASE_URL` 设为 OpenRouter 的 API 地址（如 https://openrouter.ai/api/v1），决定请求发到哪个后端。2. 把 `ANTHROPIC_AUTH_TOKEN` 设为 OpenRouter API Key，作为认证凭证。3. 把 `ANTHROPIC_API_KEY` 显式置空，防止旧的 Anthropic 登录态干扰。4. 重启 Claude Code 客户端让环境变量生效。配置完成后，用 `/status` 检查是否真的走 OpenRouter，并去 OpenRouter Activity Dashboard 验证 request_id 是否出现。如果配置正确但仍然报错，检查响应体（不仅是 HTTP 状态码）里的错误信息，区分是 Base URL 问题、认证失败还是模型不可用。",
      },
    ],
    relatedLinks: [
  ...
  // ========================================
  {
    ...
    title: "Claude Fable 5 API Guide: Model IDs and Fallbacks",
    metaDescription:
      "Configure Claude Fable 5 across Claude API, Bedrock, Vertex AI, Foundry, and OpenRouter. Check model IDs, refusal behavior, fallbacks, and billing signals.",
  ...
  // ========================================
  {
    ...
    title: "Claude Fable 5 API Providers: Claude, Bedrock, Vertex",
    metaDescription:
      "Compare Claude Fable 5 providers by model ID, API style, OpenAI compatibility, fallback support, enablement steps, and pricing checks.",
